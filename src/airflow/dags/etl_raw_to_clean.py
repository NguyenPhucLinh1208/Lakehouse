...
from common.datasets import raw_vnexpress_data_ready, clean_vnexpress_data_ready
...
with DAG(
    ...
    schedule=[raw_vnexpress_data_ready],
    catchup=False,
    ...
    @task(task_id="produce_clean_zone_signal", outlets=[clean_vnexpress_data_ready])
    def produce_clean_zone_signal(**context):
        if context["dag_run"].run_type == "manual":
            raise AirflowSkipException("Manual run. Skipping signal production for clean zone.")
        print(f"Producing dataset signal for downstream DAGs: {clean_vnexpress_data_ready.uri}")

    submit_spark_job >> produce_clean_zone_signal()

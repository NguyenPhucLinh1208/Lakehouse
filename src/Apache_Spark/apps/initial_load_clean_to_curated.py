...
from pyspark.sql.functions import (
    ...
    row_number # Thêm row_number để có thể tạo SK ổn định hơn nếu cần
)
...
# (Sao chép toàn bộ phần cấu hình SparkSession từ script populate_dim_date.py,
#  đảm bảo cả clean_catalog_name và curated_catalog_name được cấu hình)
...
# Hàm trợ giúp để đọc và ghi (giống như script ETL Raw->Clean, nhưng ghi vào Curated)
...
def write_curated_table_initial_load(df_to_write, table_name, partition_cols=None, mode="overwrite"): # Thêm mode, mặc định là overwrite
    # Sử dụng các biến toàn cục/module-level như CURATED_CATALOG_NAME, CURATED_DATABASE_NAME
...
# 2.1. DimDate - Giả định đã được populate bởi script populate_dim_date.py
...
write_curated_table_initial_load(dim_author_df, "DimAuthor")
...
write_curated_table_initial_load(dim_topic_df, "DimTopic")

# 2.4. DimSubTopic - Cần DimTopic đã được tạo SK (Surrogate Key)
print("Đang xử lý DimSubTopic...")
# Đọc lại DimTopic vừa tạo để lấy TopicKey
dim_topic_curated_df = read_clean_table("DimTopic", catalog=curated_catalog_name, db=CURATED_DATABASE_NAME)
...
dim_subtopic_df = subtopics_clean_df \
    ...
    .select(
        ...
        col("SubTopicName"),
        col("TopicKey").alias("ParentTopicKey"), # Lấy TopicKey từ DimTopic đã join
        col("TopicName").alias("ParentTopicName") # Lấy TopicName từ DimTopic đã join
    )
write_curated_table_initial_load(dim_subtopic_df, "DimSubTopic")
...
write_curated_table_initial_load(dim_keyword_df, "DimKeyword")
...
write_curated_table_initial_load(dim_referencesource_df, "DimReferenceSource")
...
write_curated_table_initial_load(dim_interactiontype_df, "DimInteractionType")

# --- 3. Xây dựng Bảng Sự kiện (Fact Tables) cho CURATED (Initial Load) ---
# Đọc lại các Dimension vừa tạo ở Curated để lấy SKs
...
fact_article_publication_df = fact_article_publication_source \
    ...
    ).na.fill({"PublicationDateKey": -1, "AuthorKey": -1, "TopicKey": -1, "SubTopicKey": -1}) # Xử lý lookup miss
write_curated_table_initial_load(fact_article_publication_df, "FactArticlePublication", partition_cols=["ArticlePublicationTimestamp"], mode="append")


# 3.2. FactArticleKeyword
print("Đang xử lý FactArticleKeyword...")
# Lấy các keys cần thiết từ FactArticlePublication vừa tạo để tránh join lại nhiều lần
fact_pub_keys = fact_article_publication_df.select("ArticleID_NK", "PublicationDateKey", "AuthorKey", "TopicKey", "SubTopicKey") \
                    .withColumnRenamed("PublicationDateKey", "ArticlePublicationDateKey") # Đổi tên cho đúng FK trong FactArticleKeyword
...
write_curated_table_initial_load(fact_article_keyword_df, "FactArticleKeyword")


# 3.3. FactArticleReference (Tương tự FactArticleKeyword)
...
write_curated_table_initial_load(fact_article_reference_df, "FactArticleReference")
...
fact_top_comment_activity_df = comments_clean_df \
    ...
    .select(
        col("ArticlePublicationDateKey"),
        col("ArticlePublicationDateKey").alias("CommentDateKey"), # Giả định CommentDate = ArticlePublicationDate
        col("ArticleID").alias("ArticleID_NK"),
...
write_curated_table_initial_load(fact_top_comment_activity_df, "FactTopCommentActivity")
...
fact_top_comment_interaction_detail_df = comment_interactions_clean_df \
    ...
    .select(
        col("ArticlePublicationDateKey"),
        col("ArticlePublicationDateKey").alias("InteractionDateKey"), # Giả định InteractionDate = ArticlePublicationDate
        col("ArticleID").alias("ArticleID_NK"),
        col("CommentID").alias("CommentID_NK"),
...
write_curated_table_initial_load(fact_top_comment_interaction_detail_df, "FactTopCommentInteractionDetail")
